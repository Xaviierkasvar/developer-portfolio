import {
  ...
  const Contact = () => {
    ...
    // Estado con los nombres de campos según el template
    ...
    const sendEmail = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!formData.user_email.trim()) {
          setErrorMessage('El campo "Email" es obligatorio.');
          return;
        }
        if (!formData.message.trim()) {
          setErrorMessage('El campo "Mensaje" es obligatorio.');
          return;
        }
      
        // Validar que el nombre solo contenga letras y espacios (incluyendo acentos)
        const nameRegex = /^[a-zA-ZÀ-ÿ\s]+$/;
        if (!nameRegex.test(formData.user_name.trim())) {
          setErrorMessage('El campo "Nombre" debe contener solo letras y espacios.');
          return;
        }
      
        // Validar que el teléfono solo contenga números
        const phoneRegex = /^[0-9]+$/;
        if (!phoneRegex.test(formData.user_phone.trim())) {
          setErrorMessage('El campo "Teléfono" debe contener solo números.');
          return;
        }
        ...
        if (!emailRegex.test(formData.user_email.trim())) {
          setErrorMessage('El campo "Email" debe ser una dirección de correo válida.');
          return;
        }
      
        // Si todas las validaciones pasan, limpiamos el error
        setErrorMessage('');
      
        if (form.current) {
          try {
            const result = await emailjs.sendForm(
              'service_2ggsnyu',      // Tu Service ID
              'template_8smmajb',      // Tu Template ID
              form.current,            // Referencia al formulario
              'CHvzAguZigGCUuxfA'       // Tu Public Key
            );
          ...
          setErrorMessage('El formulario no está disponible.');
        }
      };      
        ...
        <Box sx={{ overflowX: 'hidden' }}>
          ...
          >
            ...
            >
              {/* Mensaje de error */}
              {errorMessage && (
                <Typography sx={{ color: 'red', textAlign: 'center', mb: -9, mt: 9 }}>
                    {errorMessage}
                </Typography>
                )}
